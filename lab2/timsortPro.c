...
// Функция для получения TID (идентификатора потока)
pid_t gettid() {
    return syscall(SYS_gettid); // Используем системный вызов для получения TID
}

// Функция для вывода информации о потоке
void printThreadInfo(const char *threadName) {
    printf("Thread Name: %s, TID: %d, PID: %d\n", threadName, gettid(), getpid());
}
...
void *timSortThread(void *arg) {
    ...
    char threadName[32];
    snprintf(threadName, sizeof(threadName), "TimSortThread-%d", gettid());
    printThreadInfo(threadName);
